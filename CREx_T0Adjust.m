...
currdir= fullfile('Volumes','deepassport','Projects','Project-MorphemeB','EEG_Data',filesep);                %basic file path-need to change this. 
Condnom={cond,group};

%% LOAD SET FILES INTO EELAB SESSION

[ALLEEG, EEG, CURRENTSET, ALLCOM] = eeglab; 
currdirectory=fullfile(currdir,cond,group,filesep);
...
for Count=1:length(ALLEEG)
    ...
    savepath = fullfile('Users','bolger','Desktop','Files-T0Adjusted',filesep);
    EEG = pop_saveset( EEG, 'filename',char(title_new),'filepath',savepath);  % Saves a copy of the current resampled dataset to the current directory
    [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
    eeglab redraw
end
